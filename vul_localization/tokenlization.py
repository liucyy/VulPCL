...
def get_feature_token(data_path, embeddings, tokenizer):
    ...
    for _, item in data.iterrows():
        ...
        label = item.label
        vul_index = item.vul_index
        s_code = re.sub(r'[\f\r\t\v]+', ' ', s_code).strip()  # 替换源代码中的空格、换行符等，截断，只保留长度为512的部分
        s_tokens = ''.join(s_code.split(' '))
        s_tokens = tokenizer.tokenize(s_tokens)[:510]
        ...
        tokens2 = code_padding(tokens2, 256, embeddings=embeddings) # 初始为512
        vec_pkl.append([id, s_ids, tokens1, tokens2, label, vul_index])
        ids_seq.append([id, tmp_tokens])
        bar.update()
    bar.close()
    ids_seq_path = data_path.replace('set.pkl', 'seq.pkl')
    data_path = data_path.replace('.pkl', '_token.pkl')
    pkl.dump(vec_pkl, open(data_path, 'wb'))
    pkl.dump(ids_seq, open(ids_seq_path, 'wb'))
...
def get_dataset_split(pkl_path, pro):
    ...
    for train_idx, val_test_idx in sss.split(db, la):
        train_data = pd.DataFrame({'id':db['id'][train_idx], 'code': db['code'][train_idx], 'feature1':db['feature1'][train_idx], 'feature2':db['feature2'][train_idx], 'label':db['label'][train_idx], 'vul_index':db['vul_index'][train_idx]})
        train_data.to_pickle(train_set_path)
        val_test_data = pd.DataFrame({'id':db['id'][val_test_idx], 'code': db['code'][val_test_idx], 'feature1':db['feature1'][val_test_idx], 'feature2':db['feature2'][val_test_idx], 'label':db['label'][val_test_idx], 'vul_index':db['vul_index'][val_test_idx]})
    ...
    for val_idx, test_idx in sss_pa.split(tv, pla):
            val_data = pd.DataFrame({'id':db['id'][val_idx], 'code': db['code'][val_idx], 'feature1':db['feature1'][val_idx], 'feature2':db['feature2'][val_idx], 'label':db['label'][val_idx], 'vul_index':db['vul_index'][val_idx]})
            val_data.to_pickle(val_set_path)
            test_data = pd.DataFrame({'id':db['id'][test_idx], 'code': db['code'][test_idx], 'feature1':db['feature1'][test_idx], 'feature2':db['feature2'][test_idx], 'label':db['label'][test_idx], 'vul_index':db['vul_index'][test_idx]})
