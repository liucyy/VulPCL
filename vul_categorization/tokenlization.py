...
def get_feature_token(data_path, embeddings, tokenizer):
    '''生成函数的标记'''
    func_to_label = {}
    with open(args.p + '_' + args.cwe + '_labels.txt', 'r') as fp:
...
def get_dataset_split(pkl_path, pro):
    ...
    store_path = './vul_data/' + args.cwe + '/' + pro
...
if __name__ == '__main__':
    '''划分数据集'''
    print('[+] split dataset...')
    get_dataset_split('./vul_data/' + args.cwe + '/' + args.p + '_graph_input.pkl', args.p)

    embeddings = pd.read_pickle(args.p + '_embeddings.pkl')
    tokenizer = AutoTokenizer.from_pretrained("microsoft/codebert-base")
    root_path = './vul_data/' + args.cwe + '/' + args.p
