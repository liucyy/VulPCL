...
class CodeBert_Blstm(nn.Module):
    def __init__(self, config):
        ...
        self.embedding = nn.Embedding(config.n_vocab, config.embed, padding_idx=0)  # 处理cfg, dfg序列
        self.multi_atten = nn.MultiheadAttention(config.hidden_size*2, config.num_heads)
        self.conv = nn.Sequential(
                                  nn.Conv2d(1,config.batch_size,(8, config.embed)),
                                  nn.ReLU(),
                                  nn.MaxPool2d((2,1))
                                  )  # 训练从cfg, dfg, pdg和ast序列
        #self.CNNL = nn.Linear()
        # 从lstm中得到输出后，将out输入到以下三个Linear层中得到Q、K、V
        self.W_Q = nn.Linear(config.hidden_size*1, config.hidden_size*2, bias=False)
        self.W_K = nn.Linear(config.hidden_size*1, config.hidden_size*2, bias=False)
        self.W_V = nn.Linear(config.hidden_size*1, config.hidden_size*2, bias=False)
    ...
    def forward(self, s, x1, x2):
        ...
        out1 = out1.unsqueeze(1)  # [batch_size,1,seq_len,embedding_size]
        out1 = self.conv(out1)  # [batch_size,selq_len,hidden_size*2]
        ...
        code2 = x2.unsqueeze(1)
        out2 =self.conv(code2)
